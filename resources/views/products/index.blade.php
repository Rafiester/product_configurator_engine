        ...
        <div class="flex justify-between items-center w-full">
            ...
            <div x-data class="flex items-center space-x-3">
                <div class="p-1.5 bg-primary-soft dark:bg-primary-darkSoft rounded-xl inline-block">
                    <a href="{{ route('products.export') }}" class="inline-flex items-center justify-center px-6 py-2.5 bg-primary-DEFAULT hover:bg-primary-hover active:bg-primary-active border border-transparent rounded-lg font-semibold text-black dark:text-white focus:outline-none shadow-sm text-sm transition-colors">
                ...
                <div class="p-1.5 bg-primary-soft dark:bg-primary-darkSoft rounded-xl inline-block">
                    <a href="{{ route('products.create') }}" class="inline-flex items-center justify-center px-6 py-2.5 bg-primary-DEFAULT hover:bg-primary-hover active:bg-primary-active border border-transparent rounded-lg font-semibold text-black dark:text-white focus:outline-none shadow-sm text-sm transition-colors">
    ...
    <div class="py-6">
        <div class="w-full">
            ...
            <div class="bg-white dark:bg-dark-surface p-4 shadow-sm rounded-xl mb-6 border border-gray-200 dark:border-dark-border">
                ...
                <form method="GET" action="{{ route('products.index') }}" class="flex flex-row items-center gap-4 w-full">
                    <div class="w-full flex-[2]">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name..." class="block w-full rounded-md bg-white border-gray-300 placeholder-gray-400 text-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:placeholder-gray-500 focus:border-primary-DEFAULT focus:ring-primary-DEFAULT sm:text-sm">
                    </div>
                    ...
                    <div class="w-full flex-1">
                        <select name="configurator" class="block w-full rounded-md bg-white border-gray-300 text-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 focus:border-primary-DEFAULT focus:ring-primary-DEFAULT sm:text-sm">
                            ...
                            @foreach($allConfigurators as $conf)
                                <option value="{{ $conf->id }}" {{ request('configurator') == $conf->id ? 'selected' : '' }}>{{ $conf->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="w-full flex-1">
                        <select name="status" class="block w-full rounded-md bg-white border-gray-300 text-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 focus:border-primary-DEFAULT focus:ring-primary-DEFAULT sm:text-sm">
                            ...
                            <option value="active" {{ request('status') == 'active' ? 'selected' : '' }}>Publish</option>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-dark-border">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-dark-surface2 transition-colors">
                                ...
                                <td class="px-4 py-4 max-w-[350px]">
                                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate" title="{{ $product->name }}">
                                        {{ $product->name }}
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 text-left tabular-nums">
                                    RM {{ number_format($product->sdp, 2) }}
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 dark:text-gray-100 text-left tabular-nums">
                                    RM {{ number_format($product->total_sdp, 2) }}
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 text-left tabular-nums">
                                    RM {{ number_format($product->page_price, 2) }}
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 text-left tabular-nums">
                                    RM {{ number_format($product->srp, 2) }}
                                </td>
