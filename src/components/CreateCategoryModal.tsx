...
import { createCategoryModal } from '@/app/categories/actions';
...
const translations = {
  en: {
    ...
    categoryNamePlaceholder: 'e.g. Graphics Card',
    status: 'Status',
    statusPublish: 'Publish',
    statusDraft: 'Draft',
    cancel: 'Cancel',
    submit: 'Create Category',
    triggerButton: 'Create New Category',
    saving: 'Creating...',
    successTitle: 'Category Created',
    successMsg: 'Category "{name}" has been created successfully.',
    errorTitle: 'Error',
  ...
  id: {
    ...
    categoryNamePlaceholder: 'contoh: Kartu Grafis',
    status: 'Status',
    statusPublish: 'Publikasikan',
    statusDraft: 'Draf',
    cancel: 'Batal',
    submit: 'Buat Kategori',
    triggerButton: 'Buat Kategori Baru',
    saving: 'Membuat...',
    successTitle: 'Kategori Dibuat',
    successMsg: 'Kategori "{name}" telah berhasil dibuat.',
...
export default function CreateCategoryModal({ lang }: CreateCategoryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await createCategoryModal(formData);
      if (res.success) {
        showToast(
          'success',
          t.successTitle,
          t.successMsg.replace('{name}', res.category?.name || '')
        );
        setIsOpen(false);
        form.reset();
        router.refresh();
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          ...
          <div
            ...
            className={`fixed inset-0 bg-black/50 dark:bg-black/75 transition-opacity duration-300 ${
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="inactive">{t.statusDraft}</option>
                  </select>
                </div>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-dark-surface2 px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 border-t border-gray-200 dark:border-dark-border">
                ...
                <div className="p-0.5 bg-primary-soft dark:bg-primary-darkSoft rounded-xl inline-block">
                  <button
                    type="submit"
                    disabled={isPending}
                    className="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2 bg-primary-DEFAULT hover:bg-primary-hover dark:bg-primary-DEFAULT dark:hover:bg-primary-hover active:bg-primary-active border border-transparent rounded-lg font-semibold text-black dark:text-white shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-primary-DEFAULT transition-colors disabled:opacity-50"
                  >
